...
export default function BarcodeScanner({onBarcodeDetected, onClose, isActive}) {
    ...
    const handleIOSPWAServiceWorkerCameraFix = useCallback(async () => {
        ...
        console.log('📱 iOS PWA Barcode Scanner: Attempting service worker camera fix...');
    ...
    const restoreIOSPWAServiceWorker = useCallback(async (restoreData) => {
        if (!restoreData.needsRestore) return;

        console.log('📱 iOS PWA Barcode Scanner: Restoring service worker...');
    ...
    const initializeCameraStream = useCallback(async () => {
        console.log('📱 Starting iOS PWA-optimized camera initialization for barcode scanner with SW fix...');
    ...
    }, [isPWA, handleIOSPWAServiceWorkerCameraFix, restoreIOSPWAServiceWorker]);

    // Enhanced cleanup function with service worker restoration
    const cleanupScanner = useCallback(() => {
        console.log('🧹 Starting scanner cleanup with iOS PWA service worker restoration...');
                            ...
                            <div className="absolute inset-0 pointer-events-none" style={{zIndex: 10}}>
                                ...
                                <div className="absolute top-16 left-4 right-4 z-20">
                                    <div
                                        ...
                                        {isScanning ? (
                                            <div>
                                                <div className="font-semibold text-lg mb-1">📱 Position barcode in the
                                                    red frame
                                                </div>
                                                <div className="text-xs opacity-90">Hold steady • Ensure good lighting •
                                                    Keep barcode flat
                                                </div>
