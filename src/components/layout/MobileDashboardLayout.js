...
(function() {
    ...
    const isIOSPWA = isIOS && (window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone);
    ...
    if (existingManifest) {
        console.log('📱 Removing existing manifest link');
        existingManifest.remove();
    }

    // Create new manifest link with iOS-appropriate manifest
    const manifestLink = document.createElement('link');
    manifestLink.rel = 'manifest';

    if (isIOS) {
        // For iOS: Use browser mode to allow camera access
        manifestLink.href = '/manifest-ios.json';
        console.log('📱 Loading iOS manifest (browser mode for camera access)');
    } else {
        // For Android/other: Use standalone mode
        manifestLink.href = '/manifest.json';
        console.log('📱 Loading standard manifest (standalone mode)');
    }
    ...
    if (isIOS) {
        ...
        if (!document.querySelector('link[rel="apple-touch-icon"]')) {
            ...
            appleIcon.href = '/icon-192x192.png'; // Adjust path as needed
        ...
        console.log('📱 Added iOS-specific meta tags');
    }

    // Expose detection results globally for camera components
    window.iosPWACameraFix = {
        isIOS,
        isIOSPWA,
        manifestMode: isIOS ? 'browser' : 'standalone',
        cameraSupported: !isIOSPWA // Camera should work in browser mode
    };
